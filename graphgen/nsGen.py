...
class NSGen():
    ...
    def writeNS(self):
        ...
        self.fh = open(self.args.ns_file, "w")
        ...
        self.enclaves = nx.get_node_attributes(self.g, 'enclaves')
    ...
    def writePreamble(self, start_cmd):
        ...
        if not self.args.useContainers:
            self.fh.write(
                ...
                "tb-make-soft-vtype %s {%sdl380g3 MicroCloud}\n" % (
                    self.args.clickHardware, "sm " if self.useDPDK else "",
                ) +\
                "tb-make-soft-vtype %s {sm dl380g3}\n" % self.args.clickHardware +\
                "tb-make-soft-vtype %s {sm dl380g3 MicroCloud}\n" % self.args.cryptoHardware +\
                "tb-make-soft-vtype %s {sm dl380g3 MicroCloud}\n" % self.args.clientHardware
            )
    ...
    def writeEnclaveNodes(self, os, ct_os):
        ...
        for enclave in self.enclaves:
            ...
            if self.numServers == 1:
                ...
                self.fh.write("tb-set-hardware $server{0} {1}\n".format(enc, self.args.clientHardware))
                if not self.args.useContainers:
                ...
                for x in range(self.numServers):
                    ...
                    self.fh.write("tb-set-hardware $server{0}{1} {2}\n".format(enc, x + 1, self.args.clientHardware))
                    if not self.args.useContainers:
            ...
            for x in range(self.numClients):
                ...
                self.fh.write("tb-set-hardware $traf{0}{1} {2}\n".format(enc, x + 1, self.args.clientHardware))
            ...
            self.fh.write("tb-set-hardware $ct{0} {1}\n".format(enc,self.args.ctHardware))
            ...
            if self.useCrypto:
                ...
                for _ in neighbors:
                    if len(neighbors) == 1:
                        ...
                        self.fh.write("tb-set-hardware ${{crypto{0}}} {1}\n".format(enc, self.args.cryptoHardware))

                    else:
                        self.fh.write("set crypto%d-%d [$ns node]\n" % (enc, c))
                        self.fh.write("tb-set-node-os ${crypto%d-%d} %s\n" % (enc, c, os))
                        self.fh.write("tb-set-hardware ${{crypto{0}-{1}}} {2}\n".format(enc, c, self.args.cryptoHardware))
        ...
        self.fh.write("tb-set-hardware $vrouter {0}\n".format(self.args.clickHardware))
        
        self.fh.write("set control [$ns node]\n")
        self.fh.write("tb-set-node-os $control %s\n" % os)
        self.fh.write("tb-set-hardware $control {0}\n".format(self.args.clientHardware))
    ...
    def writeExternalNodes(self):
        ...
        for extern_node in range(1, self.numExternal+1):
            ...
            self.fh.write("tb-set-hardware $ext{0} {1}\n".format(extern_node, self.args.clientHardware))
        ...
        for lan, lan_count in zip(lans, range(1, len(lans)+1)):
            self.fh.write(
                "set lan{0} [$ns make-lan \"{1}\" {2} {3}]\n".format(
                    lan_count, lan, default_bw, default_delay,
                )
            )
            ...
            if self.useCrypto:
                for neighbors, num_neighbor in zip(self.g.neighbors(enclave), range(len(self.g.neighbors(enclave)))):
                    # shouldnt need sorting here since enclave has 'e' and is always second tuple value in link
                    ...
                    if len(self.g.neighbors(enclave)) == 1: 
                        link_str = "{0}set link{1} [$ns duplex-link $ct{1} $crypto{1} {2} {3} DropTail]\n".format(
                            link_str, lan_count, edgeInBW, edgeInDelay,
                        )
                        ...
                        link_str = "{0}set link{1}-{2} [$ns duplex-link $ct{1} ${{crypto{1}-{2}}} {3} {4} DropTail]\n".format(
                            link_str, lan_count, num_neighbor, edgeInBW, edgeInDelay,
                        )
                        elink_str = "{0}set elink{1}-{2} [$ns duplex-link ${{crypto{1}-{2}}} $vrouter {3} {4} DropTail]\n".format(
                            elink_str, lan_count, num_neighbor, edgeInBW, edgeInDelay,
                        )
                ...
                for neighbors, num_neighbor in zip(self.g.neighbors(enclave), range(len(self.g.neighbors(enclave)))):
                    ...
                    if len(self.g.neighbors(enclave)) == 1: 
                        elink_str = "{0}set elink{1} [$ns duplex-link $ct{1} $vrouter {2} {3} DropTail]\n".format(
                            elink_str, lan_count, edgeInBW, edgeInDelay,
                        )
                        multiplex_str = "{0}tb-set-multiplexed $elink{1} 1\ntb-set-noshaping $elink{1} 1\n".format(
                            multiplex_str, lan_count,
                        )
