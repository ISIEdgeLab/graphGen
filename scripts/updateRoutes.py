...
(output, error) = Popen(["ip", "rule"], stdout = PIPE, stderr = PIPE).communicate()
out = output.splitlines()
for line in out:
    tokens = line.strip("\n").split()
    for iface in ifaces:
...
if not args.stable:
    for line in in_routes:
        ...
        if tokens[0] == sn and tokens[2] != "0":
            route_to_add = 'sudo ip route add 10.%s.0.0/16 via 10.%s.%s.2 dev %s' % (tokens[1], tokens[2], subnet, ifaces[tokens[2]])
    ...
    for iface in ifaces:
        if int(iface) > 50:
            ...
            cmd = 'sudo ip route add 10.%s.%s.0/24 dev %s src 10.%s.%s.1 table rt2' % (iface, subnet, ifaces[iface], iface, subnet)
            check_call(cmd.split(), stdout = PIPE, stderr = PIPE)
            
            cmd = 'sudo ip route add 10.0.0.0/8 via 10.%s.%s.2 dev %s table rt2' % (iface, subnet, ifaces[iface])
            check_call(cmd.split(), stdout = PIPE, stderr = PIPE)
            ...
            cmd = 'sudo ip route add 10.0.0.0/8 via 10.%s.%s.2 dev %s' % (iface, subnet, ifaces[iface])
            check_call(cmd.split(), stdout = PIPE, stderr = PIPE)
